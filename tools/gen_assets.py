...
def despeckle(im, max_area=24, threshold=28):
    """Remove tiny isolated light blobs (stray marks in the source art).

    Pixels noticeably brighter than the tile background are grouped into
    connected components; components smaller than max_area px are painted
    with the median of their surrounding pixels. Only used for action keys,
    whose glyphs are large contiguous shapes.
    """
    im = im.copy()
    w, h = im.size
    px = im.load()
    # background estimate: median brightness of the tile interior border ring
    ring = [sum(px[x, y][:3]) for x in range(8, w - 8) for y in (8, h - 9)] + \
           [sum(px[x, y][:3]) for y in range(8, h - 8) for x in (8, w - 9)]
    ring.sort()
    bg = ring[len(ring) // 2]
    bright = {(x, y) for y in range(h) for x in range(w) if px[x, y][3] > 0 and sum(px[x, y][:3]) - bg > threshold * 3}
    seen = set()
    removed = 0
    for start in list(bright):
        if start in seen:
            continue
        comp, stack = [], [start]
        seen.add(start)
        ...
        if len(comp) <= max_area:
            for (x, y) in comp:
                nb = [px[x + dx, y + dy] for dx in range(-3, 4) for dy in range(-3, 4)
                      if 0 <= x + dx < w and 0 <= y + dy < h and (x + dx, y + dy) not in bright]
                if nb:
                    nb.sort(key=lambda c: sum(c[:3]))
                    px[x, y] = nb[len(nb) // 2]
            removed += len(comp)
    return im, removed

def to_rgb565(path, clean=False):
    global _MASK
    if _MASK is None:
    ...
    if clean:
        ...
        if removed:
            print(f"  despeckled {removed} px in {getattr(path, 'name', 'image')}")
...
def main():
    ...
    with inc.open("w") as h, src.open("w") as cpp:
        ...
        for ident, rel in ACTIONS:
            emit(cpp, h, ident, to_rgb565(PLUGIN / rel, clean=True))
