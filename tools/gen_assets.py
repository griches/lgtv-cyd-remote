...
# Generated tiles (no plugin art exists): HDMI inputs, drawn in the plugin's key style.
HDMI_INPUTS = [1, 2, 3, 4]
...
def render_hdmi(n):
    """A 144px key in the plugin's style: 'HDMI' over a large input number."""
    ...
    col = tuple(glyph) + (255,)
    d.text((size / 2, 40), "HDMI", font=font(30), fill=col, anchor="mm")
    d.text((size / 2, 92), str(n), font=font(78), fill=col, anchor="mm")
    return im
...
def main():
    ...
    with inc.open("w") as h, src.open("w") as cpp:
        ...
        for n in HDMI_INPUTS:
            emit(cpp, h, f"ic_hdmi{n}", to_rgb565(render_hdmi(n)))
        ...
        cpp.write("};\n")
    total = (len(ACTIONS) + len(HDMI_INPUTS) + len(APPS)) * ICON * ICON * 2
    print(f"wrote {len(ACTIONS)} action icons + {len(HDMI_INPUTS)} hdmi tiles + {len(APPS)} app icons ({total // 1024} KB) -> {src.relative_to(ROOT)}, {inc.relative_to(ROOT)}")
